...
document.addEventListener('onEventReceived', function(obj) {
  ...
  if (obj.detail.command === "PRIVMSG") { // Prevent animation for ping events - Curtis Geiger
      if (from == obj.detail.from) {
        const content = $('#log>div:last-child .message').html();
        $('#log>div:last-child').remove();
        $('#log>div:last-child .message').append("<br />" + content);
      }

      //smoothscroll
      if (from != obj.detail.from && smoothscroll == true) {
          $('#log>div').last().hide().slideToggle(600, "easeInOutQuart"); //New animation code
      }

      from = obj.detail.from;
